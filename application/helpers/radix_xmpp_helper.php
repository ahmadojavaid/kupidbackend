...
class Radix_xmpp
{
	...
	/* function to get the roseter */
	public function roster($user)
    {
		$this->auth();
        $this->_send("<iq type='set'><query xmlns='jabber:iq:roster'><item jid='".$user."' name='".$user."'></item></query></iq>");
		$roster = $this->_recv();
		$this->sendpresence($user);
		//check for the response
		if(!empty($roster['STREAM:STREAM/IQ/QUERY/ITEM']))
			return true;
		else
			return false;
		//
		//$roster=simplexml_load_string($roster);		
    }
	public function sendpresence($user){
		$this->auth();
        $this->_send("<presence to='".$user."' type='subscribe'/>");
		$roster = $this->_recv();
	}
	public function subscribed($user){
		$this->auth();
        $this->_send("<presence to='".$user."' type='subscribed'/>");
		$roster = $this->_recv();	
	}
	public function register($username,$password){
		//<iq type="set" id="U4"><query xmlns="jabber:iq:register"><username></username><password>c</password></query></iq>
		$this->_send('<iq type="set" id="U4"><query xmlns="jabber:iq:register"><username>'.$username.'</username><password>'.$password.'</password></query></iq>');
		$register = $this->_recv();
		//$roster=simplexml_load_string($roster);
		if(!empty($register['STREAM:STREAM/IQ']['attr']['TYPE']))
			return true;
		else
			return false;		
	}
